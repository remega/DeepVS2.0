...
VideoNameFile = 'Testlist.txt'
Video_dir = 'G:\database\视频库\huamwei'
CheckpointFile = './model/pretrain/'+ targetname
save_dir = 'G:\database\视频库\huamwei\DeepVS'
#

def _BatchExtraction(VideoCap, batchsize=batch_size, last_input=None, last_GT=None, video_start = True):
	if video_start:
		...
		for i in range(batchsize-overlapframe):
			_, frame = VideoCap.read()
			#print(VideoCap.get(cv2.CAP_PROP_POS_FRAMES))
			#print(frame.shape)
			frame = cv2.resize(frame, resize_shape)
			frame = frame.astype(np.float32)
			frame = frame / 255.0 * 2 - 1
...
def main():
	...
	for videonameind in namelist:
		...
		save_dir +  '\\' + VideoName + '.avi',
			cv2.VideoWriter_fourcc('D', 'I', 'V', 'X'), fps,
			VideoSize, isColor=False)
		start_time = time.time()
		losslist = np.array([])
		videostart = True
		while VideoCap.get(cv2.CAP_PROP_POS_FRAMES) < VideoFrame - framesnum - frame_skip + overlapframe - 10:
			if videostart:
			...
			for index in range(framesnum):
			   ...
			   Out_frame = cv2.resize(Out_frame, VideoSize)
