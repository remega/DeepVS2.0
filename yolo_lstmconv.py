...
class Net(BasicNet.BasicNet):
  ...
  def inference_GRU(self, videoslides, GTs, mask_in, mask_h):  # videoslides: [batch framenum h w num_features]
      with tf.variable_scope('inference'):
          ...
          with tf.variable_scope('conv_lstm', initializer=tf.random_uniform_initializer(-.01, 0.1)):
              # cell_1 = BasicConvLSTMCell.BasicConvLSTMCell([56, 56], [3, 3], 128, state_is_tuple = False)  # input size,fliter size, input channals
              # cell_2 = BasicConvLSTMCell.BasicConvLSTMCell([56, 56], [3, 3], 128, state_is_tuple = False)  # input size,fliter size, input channals
              cell_1 = BasicConvLSTMCell.BasicConvGRUCell([28, 28], [3, 3], 128,
                                                           state_is_tuple=False)  # input size,fliter size, input channals
              cell_2 = BasicConvLSTMCell.BasicConvGRUCell([28, 28], [3, 3], 128,
                                                           state_is_tuple=False)  # input size,fliter size, input channals

              new_state_1 = cell_1.zero_state(self.batch_size, 1, tf.float32)
              new_state_2 = cell_2.zero_state(self.batch_size, 1, tf.float32)
              # print(videoslides.get_shape().as_list())
          for indexframe in range(self.framenum):
              frame = videoslides[:, indexframe, ...]
              # print(indexframe+self.gapnum)
              ...
              if self.startflagcnn == True:
                  self.flowfeatures_colllection = self.pretrain_var_collection
              new_state_1 = cell_1(CNNout, new_state_1, mask_in[..., 0:3], mask_h[..., 0:3], self.dp_in, self.dp_h,
                                        'GRU_layer1')
              new_state_2 = cell_2(new_state_1, new_state_2, mask_in[..., 4:7], mask_h[..., 4:7], self.dp_in, self.dp_h,
                                        'GRU_layer2')
              deconv = self.transpose_conv_layer('deconv', new_state_2, 4, 16, stride=2, pretrain=False, trainable=True)
              deconv2 = self.transpose_conv_layer('deconv2', deconv, 4, 1, stride=2, linear=True, pretrain=False,
                                                  trainable=True)
              if self.startflagcnn == True:
                  tf.get_variable_scope().reuse_variables()
                  self.trainable_var_collection.extend(cell_1.trainable_var_collection)
  ...
  def inference_onelstm(self, videoslides, GTs, mask_in, mask_h): #videoslides: [batch framenum h w num_features]
    with tf.variable_scope('inference'):
        shapes = videoslides.get_shape().as_list()
        shapes2 = GTs.get_shape().as_list()
        assert len(shapes)==5 and len(shapes2)==5
        ...
        for indexframe in range(self.framenum):
            ...
            deconv = self.transpose_conv_layer('deconv', y_1, 4, 16, stride=2, pretrain=False, trainable=True)
            deconv2 = self.transpose_conv_layer('deconv2', deconv, 4, 1, stride=2, linear=True, pretrain=False, trainable=True)
            if self.startflagcnn == True:
                tf.get_variable_scope().reuse_variables()
                self.trainable_var_collection.extend(cell_1.trainable_var_collection)
  ...
  def inference_oneGRU(self, videoslides, GTs, mask_in, mask_h):  # videoslides: [batch framenum h w num_features]
      with tf.variable_scope('inference'):
          shapes = videoslides.get_shape().as_list()
          shapes2 = GTs.get_shape().as_list()
          assert len(shapes) == 5 and len(shapes2) == 5
          ...
          for indexframe in range(self.framenum):
             ...
             # new_state_2 = cell_2(new_state_1, new_state_2, mask_in[..., 4:7], mask_h[..., 4:7], self.dp_in, self.dp_h,
             #                           'GRU_layer2')
              deconv = self.transpose_conv_layer('deconv', new_state_1, 4, 16, stride=2, pretrain=False, trainable=True)
              deconv2 = self.transpose_conv_layer('deconv2', deconv, 4, 1, stride=2, linear=True, pretrain=False,
                                                  trainable=True)
              if self.startflagcnn == True:
                  tf.get_variable_scope().reuse_variables()
