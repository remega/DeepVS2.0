...
class BasicConvGRUCell(ConvRNNCell):
    ...
    def __call__(self, inputs, h, mask_in, mask_h, dp_in, dp_h, scope=None, dropout = 'bayes'):
        ...
        if dropout == 'bayes':
            ...
            z_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h1')
            r_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h2')
            th_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h3')


            z_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i1')
            r_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i2')
            th_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i3')
        elif dropout == 'normal':
            z_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h1')
            ...
            th_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i3')
            z_h = tf.nn.dropout(z_h, dp_h)
            r_h = tf.nn.dropout(r_h, dp_h)
            th_h = tf.nn.dropout(th_h, dp_h)

            z_in = tf.nn.dropout(z_in, dp_in)
            r_in = tf.nn.dropout(r_in, dp_in)
            th_in = tf.nn.dropout(th_in, dp_in)
...
class BasicConvLSTMCell(ConvRNNCell):
    ...
    def __call__(self, inputs, state, mask_in, mask_h, dp_in, dp_h, scope=None, dropout = 'bayes'):
        """Long short-term memory cell (LSTM)."""
        # Parameters of gates are concatenated into one multiply for efficiency.
        assert mask_in.get_shape().as_list()[:-1] == inputs.get_shape().as_list() and mask_h.get_shape().as_list()[
                                                                                      :-1] == inputs.get_shape().as_list()
        ...
        if dropout == 'bayes':
            ...
            i_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h1')
            j_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h2')
            f_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h3')
            o_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h4')
            i_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i1')
            j_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i2')
            f_in = self._conv_linear([inputs], self.filter_size, self.num_features, True,
                                     scope=scope + 'i3')
            o_in = self._conv_linear([inputs ], self.filter_size, self.num_features, True,
                                     scope=scope + 'i4')
        elif dropout == 'normal':
            i_h = self._conv_linear([h], self.filter_size, self.num_features, False,
                                    scope=scope + 'h1')
            ...
            i_h = tf.nn.dropout(i_h, dp_h)
            j_h = tf.nn.dropout(j_h, dp_h)
            f_h = tf.nn.dropout(f_h, dp_h)
            o_h = tf.nn.dropout(o_h, dp_h)
            i_in = tf.nn.dropout(i_in, dp_in)
            j_in = tf.nn.dropout(j_in, dp_in)
            f_in = tf.nn.dropout(f_in, dp_in)
            o_in = tf.nn.dropout(o_in, dp_in)
